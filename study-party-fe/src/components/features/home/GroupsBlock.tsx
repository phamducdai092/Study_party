        ...
        <Card>
            ...
            <CardContent>
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    {rooms.map((r) => (
                        <RoomCard key={r.id} room={r} enumItem={getEnum(r.topic)} onClick={() => onRoomClick?.(r)} />
                    ))}
                </div>


                <div className="mt-6 p-4 bg-gradient-to-r from-muted/30 to-muted/10 rounded-xl">
                    <h3 className="font-semibold mb-3 flex items-center gap-2"><Star className="h-4 w-4 text-primary" />Hành động nhanh</h3>
                    <QuickActions />
                </div>
            </CardContent>
        </Card>
